...
def prepare_data(data_name, args):
    ...
    out_file_prefix = f"{args.split}_{args.prompt_type}_{args.model_name_or_path.split('/')[-1]}_{args.num_test_sample}_seed{args.seed}_t{args.temperature}"
...
def main(llm, tokenizer, data_name, args):
    ...
    for example in tqdm(examples, total=len(examples)):
        ...
        if example["question"] == "":
            continue
        ground_truth_cot, ground_truth_ans = parse_ground_truth(example, data_name)
        example["ground_truth_ans"] = ground_truth_ans
    ...
    for i, sample in enumerate(samples):
        ...
        for j in range(len(preds)):
            if sample["ground_truth"] in ["A", "B", "C", "D", "E"] and preds[j] not in [
                "A",
                "B",
                "C",
                "D",
            ...
            elif is_multi_choice(sample["ground_truth"]) and not is_multi_choice(preds[j]):
